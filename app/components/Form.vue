      ...
      <div class="flex flex-col gap-6">
        ...
        <div v-if="emails.length">
          ...
          <a
            ...
            :key="email"
            class="block font-normal text-sm py-2"
            :href="`mailto:${email}`"
          >{{ email }}</a>
        </div>
    ...
    <form class="grid grid-cols-2 gap-4 font-normal flex-1" novalidate @submit.prevent="handleContactSubmit">
      <div class="flex flex-col col-span-2 lg:col-start-1 lg:col-span-1 gap-2">
        ...
        <input
          ...
          placeholder="Name" class="text-xl p-4 border-2 h-12 lg:h-14 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        />
        <p v-if="contactErrors.name" id="contact-name-error" class="text-sm text-red-300" aria-live="polite">{{ contactErrors.name }}</p>
      </div>
      <div class="flex flex-col col-span-2 lg:col-span-1 lg:col-start-2 gap-2">
        ...
        <input
          ...
          placeholder="Your Email" class="text-xl p-4 border-2 h-12 lg:h-14 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        />
        <p v-if="contactErrors.email" id="contact-email-error" class="text-sm text-red-300" aria-live="polite">{{ contactErrors.email }}</p>
      </div>
      <div class="col-span-2 flex flex-col gap-2">
        <label class="sr-only" for="contact-subject">Subject</label>
        <input
          id="contact-subject" v-model="contactForm.subject" name="subject"
          placeholder="Subject" class="text-xl p-4 border-2 col-span-2 h-12 lg:h-14 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        />
        <p v-if="contactErrors.subject" id="contact-subject-error" class="text-sm text-red-300" aria-live="polite">{{ contactErrors.subject }}</p>
      </div>
      <div class="col-span-2 flex flex-col gap-2">
        <label class="sr-only" for="contact-message">Message</label>
        <textarea
          id="contact-message" v-model="contactForm.message" name="message"
          placeholder="Messages ..." class="text-xl p-4 border-2 col-span-2 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        ></textarea>
        <p v-if="contactErrors.message" id="contact-message-error" class="text-sm text-red-300" aria-live="polite">{{ contactErrors.message }}</p>
      </div>
